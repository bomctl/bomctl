...
func Push(sbomID, pushURL string, opts *options.PushOptions) error {
	...
	if err := pushClient.AddFile(filepath.Base(pushURL), sbomID, opts); err != nil {
		return fmt.Errorf("%w", err)
	}
...
func addExternalReferenceFiles(sbomID, pushURL string, pushClient client.Client, opts *options.PushOptions) error {
	...
	for _, document := range extRefDocs {
		extRefURL := getExtRefPath(pushURL, document.Metadata.Id, document.Metadata.Name, opts)

		if err := pushClient.AddFile(extRefURL, document.Metadata.Id, opts); err != nil {
			return fmt.Errorf("%w", err)
		}
	}
...
func getExternalReferences(id string, opts *options.PushOptions) ([]*sbom.ExternalReference, error) {
	opts.Logger.Info("Fetching external reference SBOMs", "id", id)
	...
	extRefs, err := backend.GetExternalReferencesByDocumentID(id, "BOM")
	if err != nil {
		return nil, fmt.Errorf("%w", err)
	}
...
func resolveExternalReferences(refs []*sbom.ExternalReference, opts *options.PushOptions) ([]*sbom.Document, error) {
	...
	for _, ref := range refs {
		document, err := fetch.GetRemoteDocument(ref.Url, fetchOpts)
		if err != nil {
		...
		extRefs, err := getExternalReferences(document.Metadata.Id, opts)
		if err != nil {
			return nil, fmt.Errorf("%w", err)
		}
...
func getExtRefPath(destPath, docID, docName string, opts *options.PushOptions) string {
	...
	return filepath.Join(destDir, fmt.Sprintf("%s%s", fileName, ext))
}
